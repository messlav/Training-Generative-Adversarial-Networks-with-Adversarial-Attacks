# My realization of SNAN with logging
...
def train_gan(dataloader, net_D, net_G, num_epochs, loss_fn, optim_G, optim_D, sched_G, sched_D, record):
    ...
    if record:
        ...
        wandb.run.name = "decrease-noise-size-in-G"
    ...
    print('Training for batch size = {}, epochs = {} done for {:.1f} minutes'.format(batch_size, num_epochs,
                                                                                     (time.time() - start) / 60))
    wandb.finish()
